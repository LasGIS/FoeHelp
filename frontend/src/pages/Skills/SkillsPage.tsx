...
class SkillsPage extends React.Component<PropsFromRedux> {
  ...
  saveToFile = () => {
    ...
    downloadToFile(skillList, 'skills.ts', 'text/plain', 'import { SkillType } from "./dic-type";\n\nexport const SKILLS: SkillType[] = ', ';\n');
  };
  ...
  render() {
    const { searchValue, skillList, editSkill, isEditSkillShow, isNewSkill } = this.props;
    return (
      <>
        <Row style={{ marginBottom: '5px' }}>
          ...
          <div style={{ flex: 'auto' }} />
          <Button style={{ alignItems: 'end' }} type="link" icon={<PlusOutlined />} onClick={this.createNewSkill}>
            Добавить Connectivity
          </Button>
        </Row>
        <SearchInputWithDelay value={searchValue} onChange={this.setSearchValue} placeholder="Начните набирать значение любого из полей" />
        <Table
        ...
        <Modal
          ...
          width="1024px"
        >
          <SkillDetailForm isExist={!isNewSkill} editSkill={editSkill} onSave={this.saveDetail} onClose={this.detailClose} />
        </Modal>
      </>
    );
  }
